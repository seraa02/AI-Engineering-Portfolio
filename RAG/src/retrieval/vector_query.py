...
DEFAULT_EF_SEARCH = 40  # pgvector's own default -- eval/recall_at_k.py found no
# recall difference across ef_search in {10, 40, 100, 200} at this corpus size
# (749 chunks): HNSW is close enough to exact search here that ef_search has
# nothing to trade off yet. Kept explicit (not None) so the sweep script and
# this default stay the same code path, and so a larger corpus later has an
# obvious place to re-run the sweep. See README's Vector Retrieval Tuning section.
...
def vector_search(
    ...
    """
    Top-k chunks by cosine similarity to query_text. min_similarity filters
    weak matches. ef_search sets pgvector's HNSW query-time search breadth
    (hnsw.ef_search, a session-level GUC) -- higher trades latency for
    recall. None leaves pgvector's own default (40) in place. See
    eval/recall_at_k.py for how DEFAULT_EF_SEARCH was chosen.
    """
    ...
    with conn.cursor() as cur:
        if ef_search is not None:
            # Not parameterizable (SET does not accept a bound parameter) --
            # safe here because ef_search is a caller-supplied int, never
            # raw user/model text.
