...
## Vector Retrieval Tuning (recall@k)

Per the build guide's Phase 2: *"Index with HNSW, tune ef_search, and measure recall at k on a small labeled set before moving on. Do not build the router on top of retrieval you have not measured."*

**Labeled set** (`eval/recall_labels.json`, built by `eval/build_recall_labels.py`): 11 (question, gold_chunk_id) pairs, built mechanically — not hand-authored — by taking `golden_dataset.json`'s verbatim `source_evidence` quote for each single-passage question and finding the actual pgvector chunk whose stored text contains it (normalizing curly quotes and the `®`/`™` symbols the SEC HTML inserts mid-sentence). Aggregation and cross-document multi-hop items are excluded — they have no single answering chunk by construction.

**Sweep** (`eval/recall_at_k.py`, results in `eval/recall_results.json`):

| ef_search | recall@1 | recall@3 | recall@5 | recall@10 |
|---|---|---|---|---|
| 10 | 36% | 45% | 45% | 64% |
| 40 (pgvector default) | 36% | 45% | 45% | 64% |
| 100 | 36% | 45% | 45% | 64% |
| 200 | 36% | 45% | 45% | 64% |

**Finding: ef_search made zero measurable difference at this corpus size.** At 749 chunks, HNSW's approximate search is already close enough to exact that widening the search breadth doesn't surface anything more — there's no recall/latency tradeoff to tune yet. `DEFAULT_EF_SEARCH` in `src/retrieval/vector_query.py` is left at pgvector's own default (40) rather than an arbitrarily higher value, since raising it here would only cost latency for no benefit. The sweep script stays in the repo so re-running it after scaling past 6 documents (where HNSW approximation error actually starts to show up) is a one-command check, not a rebuild.

**The more important finding is the recall number itself, not which ef_search produced it: recall@5 is only 45%.** For roughly half of even these single-hop, single-fact questions, plain vector similarity search does not surface the exact passage that states the answer in its top 5 results — it retrieves other topically-related chunks (the right company, the right general subject) but not the one with the specific fact. This is independent evidence for the project's central thesis: similarity search finds *related* text, not necessarily the *correct* text, which is exactly why the graph path exists for anything the ontology can model as a structured relationship rather than a passage lookup.
...
│   ├── build_recall_labels.py     # builds a (question, gold_chunk_id) set from golden_dataset.json
│   ├── recall_labels.json          # that labeled set (11 items)
│   ├── recall_at_k.py               # ef_search sweep + recall@k measurement (Phase 2)
│   └── recall_results.json           # latest recall@k sweep output
│
└── tests/
```
...
- **Phase 1** — ✅ Knowledge graph: ontology design (from real corpus), Claude extraction, entity resolution, Neo4j writes. 3,029 raw entities → 829 canonical nodes; 2,608 relationships extracted, MERGEd down to 1,193 distinct graph edges (the same real-world fact — e.g. "AMD COMPETES_WITH Intel" — is stated in multiple chunks and correctly collapses to one edge with multiple `source_chunk_ids`, not one edge per mention). Cost: $9.65.
- **Phase 2** — ✅ Vector database: embeddings (local BAAI/bge-large-en-v1.5), pgvector, HNSW indexing. 749 chunks embedded. ef_search tuned and recall@k measured against a labeled set — see [Vector Retrieval Tuning](#vector-retrieval-tuning-recallk).
...
# Phase 2: recall@k measurement + ef_search tuning (run once after embedding;
# re-run after scaling the corpus)
python -m eval.build_recall_labels
python -m eval.recall_at_k
...
- **Vector recall@5 is only 45% on the labeled set** (see [Vector Retrieval Tuning](#vector-retrieval-tuning-recallk)) — plain similarity search frequently retrieves topically-related chunks rather than the one that actually states the answer. The hybrid system's accuracy holds up anyway mostly because most benchmark questions route to GRAPH (which doesn't depend on vector recall at all), not because vector retrieval itself is precise. This is a real weakness of the vector path in isolation, not just a graph-path strength.
...
All five phases are built, tested, and run against the real 6-filing corpus. 75 tests passing (`pytest`). See [Benchmark Results](#benchmark-results) for the headline numbers, [Vector Retrieval Tuning](#vector-retrieval-tuning-recallk) for the Phase 2 recall@k measurement, and [Known Limitations](#known-limitations) for what to look at next before scaling past 6 documents.
